...
struct SettingsView: View {
        ...
        ScrollView {
            Form {
                ...
                Section(header: Text("Translation").font(.headline)) {
                    Picker("Subtitle Backend", selection: Binding(
                ...
                Section(header: Text("Cloud Assistant").font(.headline)) {
                    SecureField("OpenRouter API Key", text: Binding(
                        get: { viewModel.settings.openRouterAPIKey },
                        set: { viewModel.updateOpenRouterAPIKey($0) }
                    ))
                    .textFieldStyle(.roundedBorder)

                    Text("Used only for cloud-side question detection and bilingual answer generation. Subtitle translation remains local/system based on the backend above.")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
    ...
    private func translationBackendHint(for backend: TranslationBackendPreference) -> String {
        switch backend {
        case .system:
            return "Uses the system translation engine for subtitle translation when available. Cloud answers still use OpenRouter separately."
        case .localOllama:
            return "Uses the local Ollama model for subtitle translation first, then falls back to the system translator when needed. Cloud answers still use OpenRouter separately."
        }
    }
}
