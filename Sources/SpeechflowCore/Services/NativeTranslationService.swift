...
public final class NativeTranslationService: ObservableObject, TranslateServicing, @unchecked Sendable {
    private let stateQueue = DispatchQueue(label: "Speechflow.NativeTranslationService")
    ...
    public func enqueue(_ segment: TranscriptSegment) {
        let (sink, continuation, queueActive) = stateQueue.sync {
            (self.eventSink, self.queueContinuation, self.isQueueActive)
        }
        debugLog("[Translation] enqueue called, text=\"\(segment.sourceText.prefix(30))\", config=\(configuration != nil), queueActive=\(queueActive), hasContinuation=\(continuation != nil), hasEventSink=\(sink != nil)")
    ...
    public func processQueue(with session: TranslationSession) async {
        ...
        let sink = stateQueue.sync { self.eventSink }

        var isPrepared = false
        for await segment in freshStream {
            debugLog("[Translation] processQueue: got segment \"\(segment.sourceText.prefix(30))\", eventSink=\(sink != nil)")
            do {
                if !isPrepared {
                ...
                sink?(.translationFailed(segmentID: segment.id, message: error.localizedDescription))
            }
        }
    }
