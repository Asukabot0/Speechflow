...
- 云端 AI 翻译/提词：`OpenRouterTranslationService`
- 并行翻译流：`ParallelTranslateService`
...
- 配置持久化：`UserDefaultsSettingsStore`
...
- 本地 Ollama 翻译
- 系统翻译 fallback 路由
- 基础分句、静音提交和稳定提交（已优化长难句切分）
- 原文与译文单句对应保存至本地日志 (`~/Documents/Speechflow_Transcript.txt`)
- 设置页的主要交互闭环

### 4.2 当前明确还没完成的事情
- 真实网络质量监控
- 更彻底的“重复前缀不再重复送翻译”控制（目前短段落合并已大幅缓解此问题）
...
3. 顶层由 `ParallelTranslateService` 分发到不同的翻译 provider
4. 字幕翻译走 `TranslationRouterService`，默认由 `LocalOllamaTranslationService` 执行
5. AI 翻译/提词走 `OpenRouterTranslationService`
6. 本地 provider 失败时按 router 逻辑回退到云端（或系统） provider
7. 翻译结果通过 `segment.id` 对应写回（支持多路翻译），不靠文本对比覆盖
...
- 稳定提交
- 基于标点和词边界的启发式切分
- 放宽 Whisper 强制切分阈值，解决长口播“憋字”导致翻译延迟十几秒的严重问题
- 在送翻前按句末标点强制断句，确保翻译按小片段快速返回
- 电影字幕风格显示（译文保持5-8行独立换行）

未完成：

- 某些罕见极长句（无任何标点和停顿）的更智能强切逻辑
...
### 8.8 Milestone H: 测试覆盖

状态：已完成基础覆盖。

已完成：

- `SpeechflowCoreTests` 目标搭建（Swift Testing 框架）
- P0 纯逻辑单元测试：
  - `TextChunkingHelperTests`（切分、标点、语义块评估）
  - `TranslationOutputNormalizerTests`（前缀清洗、代码围栏去除）
  - `TranscriptBufferTests`（提交、去重、refinement 替换、状态写回）
  - `AutoCommitSchedulerTests`（延迟计算、取消逻辑）
  - `OverlayViewModelBuilderTests`（渲染行生成、换行、翻译对齐）
- P1 集成测试：
  - `AppCoordinatorTests`（状态机转移、ASR 事件驱动、翻译失败容错）
- 共 51 个测试用例，全部通过，无外部依赖
- 修复了 `TextChunkingHelper.splitAtClauseBoundaries` 数组越界 bug
- 修复了 `TranscriptBuffer` 重复提交抑制逻辑缺陷

未完成：

- `TranslationRouterService` 集成测试（fallback 路由验证）
- UI 层（SwiftUI）测试
- 长时间 soak test 自动化
- CI/CD 集成
...
3. 补充 `TranslationRouterService` 集成测试，覆盖 fallback 路由场景。
4. 把 `StubNetworkMonitor` 换成真实网络状态源。
5. 建立 CI/CD 自动化测试流水线（`swift test` 已可直接集成）。
6. 最后再做更激进的字幕体验优化，例如更细粒度的滚动策略或更强的译文缓存。
